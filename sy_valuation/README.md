...
| 페이지 | 설명 |
|--------|------|
| **대시보드** | 주요 지수/환율 + 12개 시장 종합 뉴스 + 저평가 Top5 |
| **저평가 TOP10** | 9개 모델 가중평균 적정주가 vs 현재가, 정량필터 통과 종목 |
| **기업 가치 평가** | 기업명/코드 → 9개 모델 분포, 자동완성, Naver 실시간 재무 |
| **투자 분석** | 가치평가 + 뉴스 감성 + 변동성 → 단기 매수/매도/손절 + 장기 사유 |
| **SY 평가법** | 3접근법 (수익/자산/상대) min-mid-max 기업가치 → 주당 적정가 |
| **SY 평가법 가치평가** | 단일 종목 SY 평가법 상세 + 자동 피어 그룹 |
| **삼프로TV** ⭐ | 삼프로TV YouTube 최신 영상 + 토픽 자동 분류 |
| **토픽 뉴스** | 22 토픽 (정책/금융/산업/테마) 자동 갱신 |
...
| 환경변수 | 발급처 | 효과 |
|---|---|---|
| `DART_API_KEY` | https://opendart.fss.or.kr/ (무료) | 분기/연간 재무제표 정확 갱신 |
| `NAVER_CLIENT_ID` + `NAVER_CLIENT_SECRET` | https://developers.naver.com/apps/ (무료) | 더 정확한 한국 뉴스 (현재 Bing News 폴백) |
| `SAMPRO_CHANNEL_ID` | 이미 기본값 등록됨 (UChlv4GSd7OQl3js-jkLOnFA) | 채널 변경 가능 |
| `YT_CHANNELS` | (옵션) `이름:채널ID,이름2:채널ID2` | 추가 YouTube 채널 등록 |
| `SY_DISABLE_SCHEDULER` | `1` 시 백그라운드 prefetch 비활성화 | 디버깅/저전력 모드 |

### Render 등록 방법
1. Render Dashboard → SY Valuation 서비스 → **Environment** 탭
2. **Add Environment Variable** → Name/Value 입력 → Save
3. 자동 재배포 (1분)
